...
- **Estado**: ✅ Resuelto (commit `ce46d37`) — script anti-flash externalizado a `src/CdCSharp.BlazorUI/wwwroot/anti-flash.js` (StaticWebAsset). `BUIInitializer.razor` ahora emite `<script src="_content/CdCSharp.BlazorUI/anti-flash.js"></script>` en `<HeadContent>` (sync `<script src>` en `<head>` bloquea el body parse → mismo UX anti-flash). Estrategia descartada en favor de la actual: **plan A** (D-15) — `[Parameter] string? CspNonce` para mantener inline + nonce — añadía fricción al consumer (wire `INonceFeature`) sin ventaja sobre el archivo externo. **Plan B** elegido: zero-config consumer, `script-src 'self'` suficiente para CSP estricta, sin necesidad de `'unsafe-inline'` ni nonce. Coste: 1 HTTP request (cacheado tras 1ª visita; el archivo pesa <600 B). Verificado con `dotnet pack` Release: `staticwebassets/anti-flash.js` aparece en el `.nupkg`. 26/26 tests `BUIInitializer*` verdes.
- **Severidad**: Major
- **Esfuerzo**: M
...
- **Estado**: ✅ Resuelto (commit `7f70a33`) — `release-gate.yml` job `build-check` convertido a matrix `os: [ubuntu-latest, windows-latest]` con `fail-fast: false` y nombre dinámico `Build Check (${{ matrix.os }})`. Cada run levanta el setup completo (`setup-dotnet` con cache, `setup-node@v4`, `wasm-tools` workload) y ejecuta `dotnet build -p:TreatWarningsAsErrors=true` + `dotnet test`. Garantiza que un PR a `main` no puede mergear si Windows rompe — el bug que detonó `BLD-12` (handle locks en `dotnet clean` en Windows) habría sido capturado por este gate. `release-publish.yml` se mantiene Linux-only (criterio: el publish sigue desde una sola plataforma para evitar duplicados); el gate cubre la verificación cross-OS antes del tag.
- **Severidad**: Minor
- **Esfuerzo**: S
...
- **Estado**: ✅ Resuelto parcialmente (commit `68bdf35`, criterio 1 aplicado) — nuevo campo `private Expression<Func<TValue>>? _valueExpressionFallback;` cachea el lambda `() => Value!` con `??=` la primera vez que se entra al fallback path. La expresión captura `this` y se evalúa contra `this.Value` en runtime; la instancia es válida para toda la vida del componente. El allocation de `Expression<Func<TValue>>` (objeto de 56-72 bytes según implementación + ConstantExpression del closure) deja de ocurrir por render → 0 allocations en el path caliente. Criterio 2 (sustituir `ParameterView.FromDictionary` por algo más barato) descartado: la API de `ParameterView` no expone constructor que permita "concat" un único `ParameterValue` adicional sin alocar buffer; `FromDictionary` sigue siendo el camino soportado. El dictionary `patched` y la enumeración de `parameters` permanecen como estaban — siguen siendo el bottleneck remanente, pero su coste es ~50-100 ns mientras que la expresión cacheada elimina la asignación de objeto. Criterio 3 (medir con `IBUIPerformanceService`) queda como follow-up de PERF-09 (baseline) — el cambio es correcto por inspección y no rompe ningún test (404/404 input tests siguen verdes).
- **Severidad**: Minor
- **Esfuerzo**: S
