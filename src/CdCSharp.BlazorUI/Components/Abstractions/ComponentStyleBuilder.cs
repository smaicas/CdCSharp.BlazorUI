...
internal sealed class ComponentStyleBuilder
{
    ...
    private static string ToKebabCaseComponentName(string value)
    {
        ...
        if (value.StartsWith("UI", StringComparison.InvariantCultureIgnoreCase)) value = value[2..];
        else if (value.StartsWith("BUI", StringComparison.InvariantCultureIgnoreCase)) value = value[3..];
